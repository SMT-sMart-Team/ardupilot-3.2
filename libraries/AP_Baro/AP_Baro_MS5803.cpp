...
/// I2C SerialBus
AP_SerialBus_I2C_MS5803::AP_SerialBus_I2C_MS5803(uint8_t addr) :
    _addr(addr),
    _i2c_sem(NULL) 
{
}
    ...
    _use_timer(use_timer)
{
    ...
    if (!_serial->sem_take_blocking()){
        hal.scheduler->panic(PSTR("PANIC: AP_Baro_MS5803: failed to take serial semaphore for init"));
    }
    ...
    if (_use_timer) {
        hal.scheduler->register_timer_process( AP_HAL_MEMBERPROC(&AP_Baro_MS5803::_timer));
    }
}
 ...
 */
bool AP_Baro_MS5803::_check_crc(void)
{
    int16_t cnt;
    uint16_t n_rem;
    uint16_t crc_read;
...
*/
void AP_Baro_MS5803::_timer(void)
{
...
*/
void AP_Baro_MS5803::accumulate(void)
{
    if (!_use_timer) {
